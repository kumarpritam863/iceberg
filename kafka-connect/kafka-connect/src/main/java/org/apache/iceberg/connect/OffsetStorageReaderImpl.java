...
public class OffsetStorageReaderImpl implements CloseableOffsetStorageReader {
  ...
  public OffsetStorageReaderImpl(
    ...
    this.offsetReadFutures = Sets.newHashSet();
  }
  ...
  public <T> Map<Map<String, T>, Map<String, Object>> offsets(
    ...
    Map<ByteBuffer, Map<String, T>> serializedToOriginal = Maps.newHashMap();
    for (Map<String, T> key : partitions) {
      try {
        // Offsets are treated as schemaless, their format is only validated here (and the returned
        // value below)
        OffsetUtils.validateFormat(key);
        byte[] keySerialized =
            keyConverter.fromConnectData(namespace, null, Arrays.asList(namespace, key));
        ByteBuffer keyBuffer = (keySerialized != null) ? ByteBuffer.wrap(keySerialized) : null;
        serializedToOriginal.put(keyBuffer, key);
      } catch (Throwable t) {
        LOG.error(
            "CRITICAL: Failed to serialize partition key when getting offsets for task with "
    ...
    Map<Map<String, T>, Map<String, Object>> result = Maps.newHashMap();
    for (Map.Entry<ByteBuffer, ByteBuffer> rawEntry : raw.entrySet()) {
      try {
        ...
        if (!serializedToOriginal.containsKey(rawEntry.getKey())) {
          LOG.error(
              "Should be able to map {} back to a requested partition-offset key, backing "
        ...
        SchemaAndValue deserializedSchemaAndValue =
            valueConverter.toConnectData(
                namespace, rawEntry.getValue() != null ? rawEntry.getValue().array() : null);
        ...
        LOG.error(
            "CRITICAL: Failed to deserialize offset data when getting offsets for task with"
                + " namespace {}. No value for this data will be returned, which may break the "
